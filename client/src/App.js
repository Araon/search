  ...
  const fetchMovies = async (q) => {
    try {
      ...
      if (!isCoolSearchOn) {
        ...
        const response = await fetch(`/api/animes/semantic?q=${term}`);
        const data = await response.json();
        if (data?.animes && Array.isArray(data.animes)) {
          setanimes(data.animes);
        }
  ...
  return (
    <div className="bg-neutral-950 text-white min-h-screen flex flex-col items-center justify-center">
        ...
        <div className="space-x-3">
          ...
          >
            <input
              ...
              onChange={(e) => setIsCoolSearchOn(e.target.checked)}
            />
            <div
      ...
      <div className="mt-10 pb-10 flex flex-wrap gap-4 justify-center text-center">
        {loading
          ? [1, 2, 3, 4].map((x) => <MovieSkeletonCard key={`skel-${x}`} />)
          : null}
        {!loading && animes.length
          ? animes.map((anime) => <MovieCard anime={anime} key={anime.id} />)
          : !loading && (
              <div>
                <span>No animes found.</span>
                <br />
                <span className="text-xs leading-3">
                  Please note that the movie database not complete and quite
                  old. animes released post 2017 will not be available.
                </span>
              </div>
            )}
      </div>
